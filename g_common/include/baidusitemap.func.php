...
define('PLUS_BAIDUSITEMAP_VER','0.0.3');
...
EOT;
        ...
        ShowMsg("成功安装数据库！首次安装进行全量索引提交，此过程可能比较长，请耐心等待……".$nerrCode,"?dopost=auth&action=resubmit");
        exit();
    } else {
        return True;
    }
...
function baidu_savesitemap($action, $site, $type, $bdpwd, $sign)
{
    ...
    if($pagesize>0)
    {
        for($i=0;$i<$pagesize;$i++)
        {
            ...
            $indexurl = $siteurl."/plus/baidusitemap.php?dopost=sitemap_index&type={$script}&pwd={$bdpwd}&pagesize={$i}";
            $time=time();
