...
if($dopost=='auth'){
    if ( empty($sign) )
    {
        ...
        if($data['status']==0){
            $checkurl=$siteurl."/plus/baidusitemap.php?dopost=checkurl&checksign=".$data['checksign'];
            ...
            if($output['status']==0){
                ...
                ShowMsg("提交百度索引失败，无法校验本地密钥！","baidusitemap_main.php");
                exit();
            }
        }
    } else {
...
} elseif ( $dopost=='submit' )
{
    ...
    $indexurl = $siteurl."/plus/baidusitemap.php?dopost=sitemap_urls&pwd={$bdpwd}&type={$type}";
...
} elseif ( $dopost=='bind_site_id' )
{
    ...
    if($data['status']==0){
        ...
        if($output['status']==0){
            ...
            ShowMsg("无法校验本地密钥！","baidusitemap_main.php");
            exit();
        }
    }
				...
				<table width="98%" border="0" cellspacing="1" cellpadding="1">
					<tbody>
						...
						<tr>
							...
							<td width="84%" style="text-align:left;">	<span style='color:black'>{$setupmaxaid}</span>
							</td>
						</tr>
						<tr>
